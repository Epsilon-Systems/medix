...
class Countdown extends Field
{
	...
	{
		$responsive_css = $this->widget->getOption('responsive_css');

		// If we are not viewing in fullscreen the previewer (with responsive controls),
		// then re-generate Custom CSS, we should not render non-desktop CSS
		// due to the IFrame having small width and triggering the tablet/mobile CSS breakpoints
		if (!$this->data->get('fullscreen') && is_array($responsive_css))
		{
			unset($responsive_css['tablet']);
			unset($responsive_css['mobile']);
		}

		$custom_css = \NRFramework\Helpers\Responsive::renderResponsiveCSS($responsive_css, '.nrf-countdown.' . $this->widget->getOption('id'));

		return [
			'css' => \NRFramework\Widgets\Countdown::getCSS($this->theme),
			'js' =>  \NRFramework\Widgets\Countdown::getJS(),
			'custom_css' => $this->widget->getOption('css_vars') . $custom_css
		];
	}
