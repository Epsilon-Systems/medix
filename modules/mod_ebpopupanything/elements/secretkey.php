<?php
/**
 * @package Module EB PopUp Anything for Joomla!
...
class JFormFieldSecretkey extends JFormField {
	...
	public function getInput() {
		...
		/*Purchase Key*/
		$eb_popupanything_purchase_key = $this->form->getValue('eb_popupanything_purchase_key', 'params');
		if($eb_popupanything_purchase_key == ''){
			$eb_popupanything_purchase_key_final = '';
		} else {
			$eb_popupanything_purchase_key_final = "purchasekey=".$eb_popupanything_purchase_key."&domain=".$site_url_domain;
		}
	   	$db = JFactory::getDbo();
	    ...
	    if(!empty($update_site)){
		    $upd= "UPDATE `#__update_sites` SET `extra_query`= '".$eb_popupanything_purchase_key_final."' WHERE `update_site_id` = ".$update_site;
		    $db->setQuery($upd);
		    $db->execute();
		}	
